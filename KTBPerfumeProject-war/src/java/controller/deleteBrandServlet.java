...
public class deleteBrandServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (x.getProductCollection().isEmpty()) {
                brandFacade.remove(brandFacade.find(id));
                request.getRequestDispatcher("showBrandAdminServlet").forward(request, response);
            } else {
                request.setAttribute("error", "there still product in this brand, please delete them first");
                List<Product> list = (List<Product>) x.getProductCollection();
                request.setAttribute("listPro", list);
                request.setAttribute("b", x);
                request.getRequestDispatcher("admin_show_productB.jsp").forward(request, response);
            }
        }
    }
